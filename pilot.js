...
http.createServer(function(request, response) {
  ...
  response.end();
    ...
    request.on('data', function(data) {
      options += data
      console.log('Partial body: ' + options)
    });
    request.on('end', function() {
      console.log(parse(options));
      sendEmail(parse(options));
    });
  }
...
var sendEmail=async function (options) {
    ...
    //await tab.goto("C:\\Users\\saffalkhanna\\Downloads\\Automation-Gmail-puppeteer-master (new)\\Automation-Gmail-puppeteer-master\\stot.html");
    await tab.goto(url, {
    ...
    await tab.waitForSelector(".T-I.J-J5-Ji.T-I-KE.L3",{timeout:20000});
    console.log("compose");
    ...
    //for(let key in id){
        ...
        await tab.type(".aoD.hl",id,{delay:200});
        await tab.keyboard.press("Enter");
    //}
